      ...
      <GestureDetector gesture={gesture}>
        <Animated.View style={[styles.swipeablePost, animatedStyle]}>
          <View style={[
            ...
            {item.ridestate && item.driverid && item.passengerrhodesid === user.rhodesid && (
              <TouchableOpacity
                style={{ marginTop: 10, backgroundColor: '#A62C2C', padding: 10, borderRadius: 10 }}
                onPress={handlePayDriver}
              >
                <Text style={{ color: '#FAF2E6', fontWeight: 'bold', textAlign: 'center' }}>Pay Your Driver</Text>
              </TouchableOpacity>
            )}

            <TouchableOpacity
              onPress={() => navigation.navigate('Report', {
                reportedUser: item.passengerrhodesid,
                currentUser: user.rhodesid,
                postInfo: item
              })}
