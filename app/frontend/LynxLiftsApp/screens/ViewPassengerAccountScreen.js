...
import { View, Text, Image, ScrollView, StyleSheet, ActivityIndicator, Alert, SafeAreaView, TouchableOpacity } from 'react-native';
...
const ViewPassengerAccountScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        Alert.alert('Error', 'Could not load passenger profile.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const getAvatarSource = (filename) => {
    return avatarImages[filename?.trim()];
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        ...
        <View style={styles.detailsContainer}>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>Pronouns:</Text><Text style={styles.value}>{profile.pronouns || 'Not set'}</Text></View>
...
export default ViewPassengerAccountScreen;
